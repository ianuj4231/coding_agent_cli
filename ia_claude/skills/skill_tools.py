...
from ia_claude.skills.registry import SkillRegistry, SkillNotFoundError
...
def get_registry() -> SkillRegistry:
   ...
   skills_dir resolution order:
     1. Read "skills.skills_dir" from config.yaml  →  e.g. ".ia_claude/skills"
     2. Resolve relative to Path.cwd()             →  the target project root
        (ia_claude is invoked from the project being analysed, so cwd()
         IS the target project, not the ia_claude package itself)


   Example:
     User runs ia_claude from /home/user/my_project/
     config.yaml says skills_dir: .ia_claude/skills
     → skills are loaded from /home/user/my_project/.ia_claude/skills/
   """
   global _registry
   if _registry is None:
       skills_dir = Path.cwd() / config.get("skills", {}).get("skills_dir", ".ia_claude/skills")
...
def load_skill(name: str) -> str:
   ...
   Args:
       name: Required registered skill name. The tool-call argument must be
           named exactly ``name``, not ``skill`` or ``skill_name``.
   """
